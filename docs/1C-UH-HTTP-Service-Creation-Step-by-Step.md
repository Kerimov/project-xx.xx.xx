...
**Для /nsi/delta:** обработчик должен вызываться из **модуля обработки** (ОбработкаПорталЕЦОФ_Документы), а не из модуля HTTP-сервиса. В свойствах метода укажите: **Обработчик** — обработка «ОбработкаПорталЕЦОФ_Документы», процедура «ОбработатьПолучениеДельтыНСИ». Иначе будет ошибка «Неверная сигнатура процедуры обработчика».
