...
Функция ОбработатьСозданиеДокумента(Запрос) Экспорт
	...
	Исключение
		УстановитьОшибкуОтвет(Ответ, ОписаниеОшибки(), 500);
	КонецПопытки;
	Возврат Ответ;
КонецФункции
...
Функция ОбработатьПолучениеСтатусаДокумента(Запрос) Экспорт
	...
	Попытка
		ОбъектОбработки = Обработки.ОбработкаПорталЕЦОФ_Документы.Создать();
		ОбъектОбработки.ОбработатьПолучениеСтатусаДокумента(Запрос, Ответ);
	Исключение
		УстановитьОшибкуОтвет(Ответ, ОписаниеОшибки(), 500);
	КонецПопытки;
	Возврат Ответ;
...
Процедура ЗаполнитьОтветДельтыНСИ(Ответ)
	...
	// Подразделения организаций (владелец — Организация)
	ДобавитьЭлементыНСИПодразделений(МассивЭлементов);
...
// Подразделения организаций — пробуем разные варианты имени справочника в УХ
Процедура ДобавитьЭлементыНСИПодразделений(МассивЭлементов)
	ИменаСправочников = Новый Массив;
	ИменаСправочников.Добавить("Справочник.ПодразделенияОрганизаций");
	ИменаСправочников.Добавить("Справочник.Подразделения");
	ИменаСправочников.Добавить("Справочник.ОрганизационныеПодразделения");
	
	Для Каждого ИмяСправочника Из ИменаСправочников Цикл
		Попытка
			КоличествоДо = МассивЭлементов.Количество();
			ДобавитьЭлементыНСИСправочника(МассивЭлементов, "Department", ИмяСправочника, Ложь, "organizationId");
			Если МассивЭлементов.Количество() > КоличествоДо Тогда
				Возврат; // Успешно добавили
			КонецЕсли;
		Исключение
			// Справочник не найден — пробуем следующий
		КонецПопытки;
	КонецЦикла;
КонецПроцедуры
...
// ---------------------------------------- JSON и закрытие ответа при ошибке
Процедура УстановитьОшибкуОтвет(Ответ, СообщениеОбОшибке, КодСтатуса = 500)
...
Функция НайтиСчетУчетаПоИдентификатору(ИдентификаторСтрока) Экспорт
...
Функция НайтиСтавкуНДСПоПроценту(ПроцентИлиИдентификатор) Экспорт
