...
Процедура ЗаполнитьРеквизитыДокумента(ДокументОбъект, Данные)
	...
	Если Данные.Свойство("isUPD") Тогда
		УстановитьРеквизитДокументаЕслиЕсть(ДокументОбъект, "УПД", Данные.isUPD);
		УстановитьРеквизитДокументаЕслиЕсть(ДокументОбъект, "УниверсальныйПередаточныйДокумент", Данные.isUPD);
	КонецЕсли;
	...
	Если Данные.Свойство("items") И ТипЗнч(Данные.items) = Тип("Массив") Тогда
		...
		Для Каждого ЭлементМассива Из Данные.items Цикл
			...
			// Ставка НДС (как число или справочник)
			Если ЭлементМассива.Свойство("vatPercent") Тогда
				Попытка
					// Сначала пробуем установить как число
					ПроцентНДС = Число(ЭлементМассива.vatPercent);
					НоваяСтрока.СтавкаНДС = ПроцентНДС;
				Исключение
					Попытка
						// Если не получилось как число, пробуем найти в справочнике
			...
			Если ЭлементМассива.Свойство("accountId") И Не ПустаяСтрока(СокрЛП(Строка(ЭлементМассива.accountId))) Тогда
				Попытка
					СчетУчета = НайтиСчетПоИдентификатору(ЭлементМассива.accountId);
					Если СчетУчета <> Неопределено Тогда
						НоваяСтрока.СчетУчета = СчетУчета;
					КонецЕсли;
				Исключение
					// Реквизит СчетУчета может отсутствовать
				КонецПопытки;
			КонецЕсли;
