...
Процедура ЗаполнитьРеквизитыДокумента(ДокументОбъект, Данные)
	...
	ИдентификаторПодразделения = ?(Данные.Свойство("departmentId"), Данные.departmentId, Данные.department);
	Если Не ПустаяСтрока(СокрЛП(Строка(ИдентификаторПодразделения))) Тогда
