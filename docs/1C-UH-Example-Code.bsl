...
Процедура ОбработатьПолучениеСтатусаДокумента(Запрос, Ответ) Экспорт
	...
	Попытка
		СсылкаСтрока = "";
		// ref из параметров URL (query ?ref=... или path-параметр шаблона /documents/{ref}/status)
		Если ТипЗнч(Запрос.ПараметрыURL) = Тип("Соответствие") Тогда
			...
			КонецЕсли;
		КонецЕсли;
		// Запасной вариант: извлечь ref из пути /documents/REF/status (ПутьURL в части платформ)
		Если ПустаяСтрока(СсылкаСтрока) И ТипЗнч(Запрос.ПутьURL) = Тип("Строка") Тогда
			Путь = СокрЛП(Запрос.ПутьURL);
			// Ожидаемый вид: /documents/значение/status или documents/значение/status
			НачалоШаблона = Найти(Путь, "/documents/");
			ДлинаПрефикса = 11; // длина "/documents/"
			Если НачалоШаблона <= 0 Тогда
				НачалоШаблона = Найти(Путь, "documents/");
				ДлинаПрефикса = 10; // длина "documents/"
			КонецЕсли;
			Если НачалоШаблона > 0 Тогда
				НачалоЗначения = НачалоШаблона + ДлинаПрефикса;
				КонецЗначения = Найти(Сред(Путь, НачалоЗначения), "/status");
				Если КонецЗначения > 0 Тогда
					СсылкаСтрока = СокрЛП(Сред(Путь, НачалоЗначения, КонецЗначения - 1));
					// Декодирование %2C и т.д. при необходимости
					СсылкаСтрока = СтрЗаменить(СсылкаСтрока, "%2C", ",");
					СсылкаСтрока = СтрЗаменить(СсылкаСтрока, "%2F", "/");
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
...
Функция ПолучитьДокументПоУникальномуИдентификатору(СтрокаUUID)
	...
	Если ПустаяСтрока(СтрокаUUID) Тогда
