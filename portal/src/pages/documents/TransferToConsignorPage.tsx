...
interface TransferToConsignorPageProps {
  documentId?: string;
}

export function TransferToConsignorPage({ documentId }: TransferToConsignorPageProps = {}) {
  ...
  const updateItem = (index: number, field: keyof ConsignorItem, value: any) => {
    const updated = [...items];
  ...
  const itemColumns = [
    ...
    },
    {
      ...
      render: (_: any, record: ConsignorItem) => record.amount.toFixed(2)
    },
    {
      title: 'Действия',
      key: 'actions',
      ...
      <Space direction="vertical" size="large" style={{ width: '100%' }}>
        <Space>
          ...
          <Title level={3} style={{ margin: 0 }}>
            Передача товаров комитенту (создание)
          </Title>
        </Space>
