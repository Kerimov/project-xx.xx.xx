...
interface CashReceiptOrderPageProps {
  documentId?: string;
}

export function CashReceiptOrderPage({ documentId }: CashReceiptOrderPageProps = {}) {
      ...
      <Space direction="vertical" size="large" style={{ width: '100%' }}>
        <Space>
          ...
          <Title level={3} style={{ margin: 0 }}>
            Приходный кассовый ордер (ПКО) (создание)
          </Title>
        </Space>
        ...
        >
          ...
          >
            <Form.Item
              label="ПКО №"
              name="number"
              rules={[{ required: true, message: 'Введите номер ПКО' }]}
            >
            ...
            <Form.Item label="Основание:" name="basis" rules={[{ required: true }]}>
              <TextArea rows={2} placeholder="Укажите основание поступления" />
            </Form.Item>

            <Form.Item label="От кого получено:" name="receivedFrom" rules={[{ required: true }]}>
