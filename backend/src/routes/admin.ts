...
// Ручной запуск синхронизации только складов (отдельный сервис).
adminRouter.post('/nsi/warehouses/sync', async (_req: Request, res: Response) => {
  try {
    const result = await nsiSyncService.manualSyncWarehouses();
    res.json({ data: result });
  } catch (error: any) {
    res.status(500).json({
      error: {
